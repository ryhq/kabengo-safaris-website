...
export interface ParkImage { imageUrl: string; altText?: string; caption?: string }
...
export default function ParkDetailClient({ initialPark = null, initialImages = [] }: { initialPark?: ParkDetail | null; initialImages?: ParkImage[] }) {
  ...
  const [park, setPark] = useState<ParkDetail | null>(initialPark);
  const [images, setImages] = useState<ParkImage[]>(initialImages);
  const [activities, setActivities] = useState<ActivityItem[]>([]);
  const [safaris, setSafaris] = useState<SafariItem[]>([]);
  const [loading, setLoading] = useState(!initialPark);
  const [moreOpen, setMoreOpen] = useState(false);
  const [openFaq, setOpenFaq] = useState(0);

  useEffect(() => {
    if (initialPark) return; // already server-rendered
    let alive = true;
    setLoading(true);
