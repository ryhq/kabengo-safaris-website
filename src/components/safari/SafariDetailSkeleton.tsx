...
export default function SafariDetailSkeleton() {
  return (
    <div>
      {/* Hero */}
      ...
      <div className="bg-brand-cream py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`h-8 bg-stone-200 rounded-full w-48 mb-6 ${shimmer}`} />
          <div className="space-y-4">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className={`h-40 bg-white rounded-2xl ${shimmer}`} />
            ))}
          </div>
        </div>
      </div>
